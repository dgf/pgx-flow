...
DO $$
  ...
  BEGIN
    ...
    BEGIN
      ...
      INSERT INTO input (process, data) VALUES ('parallel.process', '{"parallelize":"this"}') RETURNING uid INTO pid;

      -- assert waiting activity
      SELECT count(*) FROM state WHERE instance = pid AND await = true AND activity = 'parallel.task2b2g' INTO acts;
      IF acts = 1 THEN
      ...
      SELECT gates FROM branch WHERE instance = pid AND num = 0 INTO acts;
      IF acts = 2 THEN
      ...
      SELECT b.gates FROM state s JOIN branch b ON b.num = s.branch WHERE b.instance = pid AND s.instance = pid AND s.activity = 'parallel.gateway2' INTO acts;
      IF acts = 2 THEN
    ...
    BEGIN
      ...
      UPDATE task SET status = 'done', data = '{"confirm":"done"}' WHERE instance = pid AND activity = 'parallel.task2b2g';

      -- assert join1 on root branch
      SELECT count(*) FROM state WHERE instance = pid AND branch = 0 AND activity = 'parallel.join1' INTO acts;
      IF acts = 1 THEN
      ...
      SELECT branch FROM state WHERE instance = pid AND activity = 'parallel.gateway2' INTO acts;
      SELECT count(*) FROM state WHERE instance = pid AND branch = acts AND activity = 'parallel.join2' INTO acts;
      IF acts = 1 THEN
